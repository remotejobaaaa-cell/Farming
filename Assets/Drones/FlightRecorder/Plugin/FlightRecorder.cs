	...
	public class FlightRecorder : MonoBehaviour
	{
		...
		IEnumerator Coroutine_Recording()
		{
			...
			while (_recording)
			{
				...
				string velocityX = PlaybackScript.GetComponent<Rigidbody>().linearVelocity.x.ToString();
				string velocityY = PlaybackScript.GetComponent<Rigidbody>().linearVelocity.y.ToString();
				string velocityZ = PlaybackScript.GetComponent<Rigidbody>().linearVelocity.z.ToString();
				string _dataToAdd =
					"(" + posX + "," + posY + "," + posZ + ")" + "#" +
		...
		IEnumerator Coroutine_Playback(DataReadingStructure[] dataFromSaveFile, FlightRecorderPlayback flightRecorderPlayback, Action callback)
		{
			...
			while (i < dataFromSaveFile.Length - 1)
			{
				...
				flightRecorderPlayback.GetComponent<Rigidbody>().linearVelocity = dataFromSaveFile[i].Velocity;
				i++;
			}
